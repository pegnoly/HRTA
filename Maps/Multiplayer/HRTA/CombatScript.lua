...
--  end
  ...
  -- Спеца Аларика
  if IsNamedHero(ally_hero, 'Alaric') then
    if type == CREATURE_PRIEST or type == CREATURE_ZEALOT then
      local currentMana = GetUnitManaPoints(unit)
      local manaRestore = 1 + (GetCreatureNumber(unit)/10)

      local newMana = currentMana + manaRestore
      local maxMana = 0

      if type == CREATURE_PRIEST then
        maxMana = 12
      elseif type == CREATURE_ZEALOT then
        maxMana = 18
      end
...
-- Спеца Джалиб
function TanSpec(side, mainHeroName)
  print "TanSpec"
  local manaGenieTan = 20;
    
  for i, creature in GetCreatures(side) do
    local type = GetCreatureType(creature)
    
    if type == CREATURE_GENIE then
      SetUnitManaPoints(creature, manaGenieTan)
